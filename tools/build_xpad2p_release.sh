...
LOCK="$ROOT/xpad2p-release.lock.json"
...
HOST_CONTROLLER=${3:-"$ROOT/build/xpad2p-ionstack-reroot"}
NAME="xpad2p-ionstack-poc-$VERSION-$PLATFORM"
OUT="$ROOT/dist/$NAME"
...
rm -rf "$OUT"
rm -f "$ZIP" "$ZIP.sha256"
mkdir -p "$OUT/build/xpad2p" "$OUT/profiles" "$OUT/tools"
...
jq -r '.artifacts[] | [.source,.filename,(.size|tostring),.sha256] | @tsv' \
  "$LOCK" | while IFS="$(printf '\t')" read -r source filename expected_size expected_sha; do
    ...
    cp "$input" "$OUT/build/xpad2p/$filename"
  done

printf 'xpad2p\n' > "$OUT/build/xpad2p/.profile"
case "$PLATFORM" in
  windows-*) host_name="xpad2p-ionstack-reroot.exe" ;;
  *) host_name="xpad2p-ionstack-reroot" ;;
esac
cp "$HOST_CONTROLLER" "$OUT/$host_name"
chmod 0755 "$OUT/$host_name"
cp "$ROOT/profiles/xpad2p_profiles.json" "$OUT/profiles/"
...
cp "$ROOT/README.md" "$ROOT/LICENSE" "$ROOT/NOTICE" "$ROOT/SECURITY.md" \
  "$ROOT/xpad2p-release.lock.json" "$OUT/"
...
printf 'XPAD2P_RELEASE_OK directory=%s zip=%s commit=%s platform=%s\n' \
