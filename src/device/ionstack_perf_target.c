...
static int collect_once(unsigned sample_ms, unsigned sample_freq,
  ...
  if (workload == WORKLOAD_ASHMEM_IOCTL) {
    workload_fd = open(ashmem_path, O_RDWR | O_CLOEXEC);
    if (workload_fd < 0) {
      fprintf(stderr, "[perf-target] open %s failed errno=%d (%s)\n",
              ashmem_path, errno, strerror(errno));
      return -1;
    }
  } else if (workload == WORKLOAD_SELINUX_ENFORCE) {
    workload_fd =
        open("/sys/fs/selinux/enforce", O_RDONLY | O_CLOEXEC);
    if (workload_fd < 0) {
      fprintf(stderr,
  ...
  do {
    for (unsigned i = 0; i < burst; ++i) {
      if (workload == WORKLOAD_GETUID) {
        ...
        if (ioctl(workload_fd, ASHMEM_SET_NAME, ashmem_name) != 0) {
          failures++;
        }
      } else if (workload == WORKLOAD_SELINUX_ENFORCE) {
        char value[2];
        if (pread(workload_fd, value, sizeof(value), 0) <= 0) {
          failures++;
        }
      } else {
        int fd_once = open(ashmem_path, O_RDWR | O_CLOEXEC);
        if (fd_once < 0) {
          failures++;
        } else if (close(fd_once) != 0) {
          failures++;
        }
      }
    }
...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    } else if (strncmp(argv[i], "--ashmem-path=", 14) == 0 &&
               argv[i][14] != '\0') {
